...
Docker:
```sh
$ docker run -e POSTGRES_HOST=postgres -e POSTGRES_DB=dbname -e POSTGRES_USER=user -e POSTGRES_PASSWORD=password  kolahzary/postgres-backup-local
```
...
services:
    ...
    pgbackups:
        image: kolahzary/postgres-backup-local
        restart: always
        volumes:
...
```sh
$ docker run -e POSTGRES_HOST=postgres -e POSTGRES_DB=dbname -e POSTGRES_USER=user -e POSTGRES_PASSWORD=password  kolahzary/postgres-backup-local /backup.sh
```
